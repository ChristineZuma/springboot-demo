...
import cm.itcast.config.MinioConfig;
import cm.itcast.config.UploadFile;
import cm.itcast.pojo.Result;
import cm.itcast.service.MinioService;
...
public class MinioController {
    ...
    public Result download(HttpServletResponse response, @RequestParam("fileName") String fileName, @RequestParam("bucketName") String bucketName, @RequestParam(value = "fileId",required = false) String fileId) {
        try {
            //获取存储捅名
    ...
    public Result getFileList(@RequestParam("bucketName") String bucketName) throws IOException, InvalidKeyException, InvalidResponseException, InsufficientDataException, NoSuchAlgorithmException, InternalException, XmlParserException, InvalidBucketNameException, ErrorResponseException {
        try {
            //获取存储捅名
            bucketName = StringUtils.isNotBlank(bucketName) ? bucketName : minioConfig.getBucketName();
            //列出存储桶中所有对象
            ...
            return Result.ofSuccess(items);
        } catch (Exception e) {
            log.error("获取文件列表失败,错误信息: " + e.getMessage());
            return Result.ofSuccess("获取文件列表失败");
        }
    }

    @PostMapping("/preview")
    public Result getPreviewFile(@RequestParam("bucketName") String bucketName,
    ...
    public Result getPreviewList(@RequestParam("bucketName") String bucketName,
                                               @RequestParam("expires") Integer expires
        ...
        try {
            ...
            return Result.ofSuccess("生成可以预览的文件链接失败,错误信息:" + e.getMessage());
        }
